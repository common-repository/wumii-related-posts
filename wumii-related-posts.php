...
END;
            ...
            if ($wp_query->current_post + 1 == $wp_query->post_count) {
            	if ($this->getAdminOption(self::SCRIPT_IN_FOOTER)) {
                    ...
                    $newContent .= $this->createWumiiScript($post);
                    return $newContent;
           	}
            }
        ...
        function addFooter() {
            echo $this->createWumiiScript($this->post);
        }
        ...
        function echoWumiiScript($post) {
            echo $this->createWumiiScript($post);
        }
        
        function createWumiiScript($post = null) {
            ...
            $categories = $this->getCategoriesInJson($post);
...
<p style="margin:0;padding:0;height:1px;overflow:hidden;">
    <script type="text/javascript">
        ...
        var wumiiCategories = $categories;
    </script>
    <script type="text/javascript" src="$server/ext/relatedItemsWidget"></script><a href="http://www.wumii.com/widget/relatedItems" style="border:0;"><img src="http://static.wumii.cn/images/pixel.png" alt="无觅相关文章插件，快速提升流量" style="border:0;padding:0;margin:0;" /></a>
</p>
END;
            return $script;
        }
        
        private function getCategoriesInJson($post) {
            $categories = get_the_category(empty($post)? false : $post);
            $categoryArr = array();
            if ($categories) {
